...
at_exeCmdCipstatus(uint8_t id)
{
	...
	for (i = 0; i < at_linkMax; i++)
	{
		if (pLink[i].linkEn)
		{
			...
			{
				os_sprintf(temp, "%s:%d,\"TCP\",\"%d.%d.%d.%d\",%d,%d\r\n", at_fun[id].at_cmdName, pLink[i].linkId, IP2STR(pLink[i].pCon->proto.tcp->remote_ip), pLink[i].pCon->proto.tcp->remote_port, pLink[i].teType);
				uart0_sendStr(temp);
			}
			else
			{
				os_sprintf(temp, "%s:%d,\"UDP\",\"%d.%d.%d.%d\",%d,%d,%d\r\n", at_fun[id].at_cmdName, pLink[i].linkId, IP2STR(pLink[i].pCon->proto.udp->remote_ip), pLink[i].pCon->proto.udp->remote_port, pLink[i].pCon->proto.udp->local_port, pLink[i].teType);
				uart0_sendStr(temp);
			}
		}
	}
...
at_tcpclient_recv(void *arg, char *pdata, unsigned short len)
{
	...
	if (g_comProtocolMode & COM_OUTBOUND_FRAME)
	{
		// send data as frame
		response_IPD_handler(arg, pdata, len);
	}
	else
	{
		if (at_ipMux)
...
//  os_memcpy(devkey, esp_param.devkey, 40);
	...
	os_sprintf(upServer->url, "GET /v1/device/rom/?action=download_rom&version=%s&filename=%s HTTP/1.1\r\nHost: "IPSTR":%d\r\n"pheadbuffer"", upServer->upgrade_version, user_bin, IP2STR(upServer->ip), upServer->port, KEY);
