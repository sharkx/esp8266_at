...
at_recvTask(os_event_t *events)
{
	...
	while (READ_PERI_REG(UART_STATUS(UART0)) & (UART_RXFIFO_CNT << UART_RXFIFO_CNT_S))
	{
		...
		switch (at_state)
		{
		case statIdle: //serch "AT" head
			if ((g_comProtocolMode & COM_INBOUND_FRAME) && (temp == FRAME_MAGIC))
			{
				// start receiving frame
				at_state = frame_statRecv;
				nFrameIndexPtr = 0;
				dataFrameIn[nFrameIndexPtr++] = temp;
				frameCheckSum = temp;
			}
			else if (g_comProtocolMode & COM_INBOUND_AT)
			{
				// look for AT
				atHead[0] = atHead[1];
				atHead[1] = temp;
		...
		case frame_statRecv :
			...
			if (nBytesToGo > 0)
			{
				...
				if (nBytesToGo == 0)
				{
					...
					if (frameCheckSum == 0)
					{
						// frame is valid; process
						processFrame(dataFrameIn);
					}
					else
					{
			...
			else if (nFrameIndexPtr == FRAME_POS_PAYLOAD_IN)
			{
				// we have the frame length
				nBytesToGo = getFrameSize(dataFrameIn);
				// there is also the checksum
				nBytesToGo++;
			}
			else
			{
				// wait for the next byte; nothing to process now
			}
